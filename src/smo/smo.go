...
func (m *SMO) BuildClassifier(insts data.Instances) {
	...
	for i := range subsets {
		subsets[i] = data.NewInstancesWithInst(insts, insts.NumClasses())
	}
	...
	for i := 0; i < insts.NumClasses(); i++ {
		for j := i + 1; j < insts.NumClasses(); j++ {
			...
			_data := data.NewInstancesWithInst(insts, insts.NumClasses())
			for k := 0; k < subsets[i].NumInstances(); k++ {
				_data.Add(*subsets[i].Instance(k))
			}
			for k := 0; k < subsets[j].NumInstances(); k++ {
				_data.Add(*subsets[j].Instance(k))
			}
	...
	if numFolds <= 0 {
		...
		for i := 0; i < numFolds; i++ {
			...
			for j := 0; j < test.NumInstances(); j++ {
				vals := make([]float64, 2)
				vals[0] = m.SVMOutput(-1, *test.Instance(j))
